...
ssize_t Rio::readn(int fd, void *ptr, size_t nbytes)
{
    ...
    if ((n = rio_readn(fd, ptr, nbytes)) < 0)
        Log::unix_error("Rio_readn error");
    return n;
}

void Rio::writen(int fd, void *usrbuf, size_t n)
{
    if (rio_writen(fd, usrbuf, n) != n)
        Log::unix_error("Rio_writen error");
}
...
ssize_t Rio::readnb(rio_t *rp, void *usrbuf, size_t n)
{
    ...
    if ((rc = rio_readnb(rp, usrbuf, n)) < 0)
        Log::unix_error("Rio_readnb error");
    return rc;
}
...
ssize_t Rio::readlineb(rio_t *rp, void *usrbuf, size_t maxlen)
{
    ...
    if ((rc = rio_readlineb(rp, usrbuf, maxlen)) < 0)
        Log::unix_error("Rio_readlineb error");
    return rc;
}